...
The bot uses an environment file named `.env` to configure it. This file must be named exactly `.env` and placed in the same directory as `swarmsdiscord.py`. Within this file, you need to fill in your `OPENAI_TOKEN`, `DISCORD_TOKEN`, `DEBUG_SERVER`, and `DEBUG_CHANNEL`, and `ALLOWED_GUILDS` to get the bot to work. There are also many other configurable options, an example `.env` file is shown below.  
```shell  
...
[Instructions on how to get a server from start to finish.](https://github.com/kyegomez/SwarmsDiscord/blob/main/detailed_guides/DROPLET-GUIDE.md)
...
```shell
screen swarmsdiscord  
```

#### Hit `Ctrl+a` then `d` to detach from the running bot. The bot's screen session can be reattached: `screen -r`

  
If the last few commands don't allow the bot to run `screen swarmsdiscord`, you can attempt to run the bot another way:  

```  shell
# First, navigate to the folder where the project files are 
screen -dmS GPTBot bash -c 'python3.9 swarmsdiscord.py'  
...
```shell
docker run -d --name swarmsdiscord -v env_file:/opt/swarmsdiscord/etc/environment -v /containers/swarmsdiscord:/data -v /containers/swarmsdiscord/share:/data/share ghcr.io/kyegomez/SwarmsDiscord:main-full  
```  
...
```shell
# build the image
docker build --build-arg FULL=true -t swarmsdiscord .
# run it
docker run -d --name swarmsdiscord -v env_file:/opt/swarmsdiscord/etc/environment -v /containers/swarmsdiscord:/data -v /containers/swarmsdiscord/share:/data/share swarmsdiscord
```
...
To start the swarmsdiscord container with Docker Compose, follow these steps:  
...
To stop the swarmsdiscord container, run the following command:  
...
That's it! With these simple steps, you can start and stop the swarmsdiscord container using Docker Compose.  
...
To update the bot, run (when working in the directory of SwarmsDiscord):  
