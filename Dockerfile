...
RUN mkdir -p /opt/swarmsdiscord/etc
COPY swarmsdiscord.py /opt/swarmsdiscord/bin/
COPY image_optimizer_pretext.txt language_detection_pretext.txt conversation_starter_pretext.txt conversation_starter_pretext_minimal.txt /opt/swarmsdiscord/share/
COPY openers /opt/swarmsdiscord/share/openers
CMD ["python3", "/opt/swarmsdiscord/bin/swarmsdiscord.py"]
